...
class CustomPrettyLogger extends Interceptor {
  /// Print curl [Options]
  final bool curl;
  ...
  CustomPrettyLogger(
      {this.curl = true, this.request = true, this.requestHeader = false, this.requestBody = false, this.responseHeader = false, this.responseBody = true, this.error = true, this.maxWidth = 90, this.compact = true, this.logPrint = print});

  @override
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    logPrint('CURL =>\n');
    final String curl = Curl.curlOf(url: options.uri.toString(), method: options.method, header: options.headers.isNotEmpty ? options.headers.map((key, value) => MapEntry(key, value.toString())) : {}, body: options.data is Map<String, dynamic> ? options.data :{});
