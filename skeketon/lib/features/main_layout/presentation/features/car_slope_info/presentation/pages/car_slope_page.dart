...
class CarSlopePage extends ConsumerStatefulWidget {
  const CarSlopePage({super.key});

  @override
  ConsumerState<CarSlopePage> createState() => _CarSlopePageState();
}
...
class _CarSlopePageState extends ConsumerState<CarSlopePage> {
  ...
  Widget build(BuildContext context) {
    final carSlopeState = ref.watch(carSlopeProvider);

    return Scaffold(
      appBar: AppBar(title: const Text('Car Slope Detection')),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    switch (carSlopeState) {
                      CarSlopeSuccess(:final isListening) => Text(isListening ? 'در حال listening...' : 'Listening متوقف شده'),
                      CarSlopeLoading() => const Text('در حال بارگذاری...'),
                      CarSlopeInit() => const Text('آماده'),
                      CarSlopeFailed(:final message) => Text('خطا: $message'),
                      CarSlopeState() => const SizedBox(),
                    },
                  ],
                ),
              ),
            ...
            // دکمه‌های عملیات
            ...switch (carSlopeState) {
              CarSlopeSuccess(:final isListening) => [
                _buildActionButton(
                  isListening ? 'توقف Listening' : 'شروع Listening',
                  isListening ? Icons.stop : Icons.play_arrow,
                  isListening ? () => ref.read(carSlopeProvider.notifier).stopListening() : () => ref.read(carSlopeProvider.notifier).startListening(),
            ...
            // نمایش داده‌های شیب
            ...switch (carSlopeState) {
              CarSlopeSuccess(:final slopeData) when slopeData != null => [
                _buildSlopeDirectionCard(slopeData),
                const SizedBox(height: 16),
              ...
              CarSlopeInit() => [
                const Card(
                  child: Padding(
                    padding: EdgeInsets.all(16),
                    child: Center(child: Text('آماده برای شروع')),
                  ),
                ),
              ],
              _ => [],
